...
MWE.ensureMemberLoginModal = function() {
  ...
  modal.innerHTML = `
    <div class="member-auth-card">
      ...
      <button class="google-auth-fast-btn" type="button" data-member-google-auth>
        <svg width="18" height="18" viewBox="0 0 24 24" aria-hidden="true">
          <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
          <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
          <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18A10.996 10.996 0 0 0 1 12c0 1.77.42 3.45 1.18 4.93l3.66-2.84z"/>
  ...
  const getSafeDestination = () => {
    const fallback = "app.html?view=directory";
    const destination = modal.dataset.destination || fallback;
    try {
      const safeDestination = new URL(destination, window.location.href);
      if (safeDestination.origin !== window.location.origin) return fallback;
      return `${safeDestination.pathname}${safeDestination.search}${safeDestination.hash}`;
    } catch {
      return fallback;
    }
  };

  modal.querySelector("[data-member-google-auth]")?.addEventListener("click", () => {
...
function initPrivateAppAuth() {
  ...
  if (app === "church" && new URLSearchParams(window.location.search).get("google_auth") === "creator" && window.MWEAuth) {
    window.MWEAuth.session().then(async result => {
      if (!result.ok) return;
      if (!result.user?.isCreator) result = await window.MWEAuth.creatorUpgrade();
      if (!result.ok) { showToast(result.error || "We could not finish setting up your Creator account."); return; }
...
MWE.startGoogleAuth = function(destination) {
  const fallback = `${window.location.pathname}${window.location.search}${window.location.hash}`;
  let next = destination || fallback;
  try {
    const safeDestination = new URL(next, window.location.href);
...
MWE.handleGoogleAuthFast = function(destination) {
  MWE.startGoogleAuth(destination);
};

MWE.addGoogleAuthOptions = function(root = document) {
  root.querySelectorAll?.("[data-google-auth-surface]").forEach(form => {
    if (form.querySelector("[data-google-auth-option]")) return;
    const next = form.dataset.googleAuthNext || `${window.location.pathname}${window.location.search}${window.location.hash}`;
    const option = document.createElement("button");
    option.type = "button";
    option.className = "google-auth-fast-btn";
    option.dataset.googleAuthOption = "true";
    option.innerHTML = `
      <svg width="18" height="18" viewBox="0 0 24 24" aria-hidden="true">
        <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
        <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
      ...
      Continue with Google
    `;
    option.addEventListener("click", () => MWE.startGoogleAuth(next));
    const divider = document.createElement("div");
    divider.className = "auth-divider";
    divider.dataset.googleAuthOption = "true";
    divider.innerHTML = "<span>or with email</span>";
    const firstField = form.querySelector("label, .form-group, .form-field, div");
    form.insertBefore(option, firstField || form.firstChild);
    form.insertBefore(divider, firstField || form.firstChild);
  });
  createIcons();
};

if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", () => MWE.addGoogleAuthOptions());
} else {
  MWE.addGoogleAuthOptions();
}
...
MWE.openHostAuthModal = function(onSuccessCallback = null) {
  ...
  modal.innerHTML = `
    <div class="dash-panel dash-panel-pad host-auth-panel" style="max-width: 490px; width: 92%; margin: 30px auto; max-height: 90vh; overflow-y: auto; border-radius: 24px; box-shadow: 0 25px 60px rgba(0,0,0,0.3);">
      ...
      <!-- Registration Form -->
      <form id="host-signup-form" data-google-auth-surface onsubmit="MWE.handleHostSignup(event)">
        <div class="compact-grid">
      ...
      <form id="host-signin-form" data-google-auth-surface style="display: none;" onsubmit="MWE.handleHostSignin(event)">
  ...
  document.body.appendChild(modal);
  MWE.addGoogleAuthOptions?.(modal);
  if (typeof createIcons === "function") createIcons();
