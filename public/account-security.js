(async function(){
 ...
 async function draw(){const user=window.MWEPlatform.session;$('security-controls').hidden=!user;$('security-signin').hidden=!!user;if(!user)return;const info=await api('auth/security');$('security-summary').textContent=user.email+' · Email '+(info.emailVerified?'verified':'unverified')+' · Authenticator '+(info.mfaEnabled?'enabled':'disabled');$('security-verify').disabled=info.emailVerified||!info.emailConfigured;$('security-enroll').disabled=!info.emailVerified||info.mfaEnabled;$('security-sessions').textContent=info.sessions.length+' active session(s).';if(!info.emailConfigured)notify('Verification and recovery await account email configuration.');}
