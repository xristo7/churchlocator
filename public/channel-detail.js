(function initializeChannelDetail() {
  ...
  await window.MWEPlatform?.ready;
    ...
    root.innerHTML = `<article class="channel-detail-hero channel-detail-modern">
      ...
      <div class="channel-detail-profile"><img src="${data().escapeHtml(channel.avatar)}" alt="${data().escapeHtml(channel.owner)}" /><div class="channel-detail-identity"><h1>${data().escapeHtml(channel.name)} ${channel.verified ? `<i data-lucide="badge-check"></i>` : ""}</h1><p>${data().escapeHtml(channel.handle)} <span>·</span> Created by ${data().escapeHtml(channel.owner)}</p></div><button class="content-love-button content-love-detail" type="button" data-love-type="channel" data-love-id="${data().escapeHtml(channel.id)}" aria-pressed="false"><i data-lucide="heart"></i><span data-love-count>0</span></button><a href="messages.html?compose=channel&id=${encodeURIComponent(channel.id)}"><i data-lucide="message-circle"></i> Get in touch</a></div>
