(async function () {
  ...
  const kinds = {
    ...
    meditation: { label:"Meditation room", plural:"meditation rooms", icon:"sparkles", description:"Create a guided Christian sanctuary with a distinct atmosphere, prayer purpose, and contemplative rhythm.", sections:[
      ...
      ["Sanctuary design","Choose the visual template and atmosphere visitors will experience.",[ ["template","Room style","select",true,["timer|Be Still - countdown timer","ripple|Breath Prayer - glowing ripples","journey|Prayer Journey - four milestones","nature|Nature Teaching - guided audio","sunburst|Joy & Praise - sunburst"]], ["theme","Atmosphere","select",true,["chapel|Quiet chapel","forest|Forest stillness","ocean|Open water","sunrise|Sunrise glow","stream|Living stream","mountains|Mountain refuge","stars|Night sky"]], ["themeColor","Accent color","color",true,null,"Choose a color from the palette or open the custom picker."], ["durationMinutes","Session length in minutes","number",true,null,"Use 0 for an open-ended session."], ["timeMode","Session mode","select",true,["timed|Timed contemplation","teaching|Teaching / guided audio"]], ["toneFreq","Tone frequency","number",false,null,"Optional resonance used by the room audio."], ["selectedAudio","Opening audio","select",true,["bible|Audio Bible","instrumental|Soaking instrumental","worship|Christian worship","sermon|Spoken teaching","silence|Silence and ambience"]], ["autoPlayInterval","Auto-advance interval (seconds)","number",false,null,"Used for scripture or guided progression."], ["allowUserNavigation","Let visitors navigate freely","check"], ["commentsEnabled","Enable live comments","check"] ]],
      ["Style direction","These controls preserve what makes each room style different. Complete the fields that belong to the selected style.",[ ["inhaleWord","Ripple inhale word","text",false,null,"Breath Prayer only - for example Jesus"], ["exhaleWord","Ripple exhale word","text",false,null,"Breath Prayer only - for example Give Me Peace"], ["journeySteps","Prayer journey stages","journey",false,null,"Prayer Journey only - name each stage, add a prompt, and optionally include a scripture."], ["ambience","Nature soundscape","ambience",false,null,"Set the balance of rain, stream, fire, and breeze without writing code."], ["audioSrc","Room MP3","audio",false,null,"Optional custom room sound. MP3 only, up to 10 MB."], ["icon","Room symbol","select",false,["sparkles|Sparkles","sun|Sun","heart|Heart","cross|Cross","shield|Shield","book-open|Open Bible","moon|Moon","mountain|Mountain"],"Choose the symbol visitors will see."], ["cover","Room cover image","image",false,null,"Choose a picture from your device."], ["verses","Featured scriptures","verses",false,null,"Add up to three scripture passages with a topic, text, and reference." ] ]]
    ], defaults:{ category:"featured", purpose:"prayer", template:"timer", theme:"chapel", themeColor:"#4a5d3f", durationMinutes:20, timeMode:"timed", toneFreq:432, selectedAudio:"bible", autoPlayInterval:300, allowUserNavigation:true, commentsEnabled:false, inhaleWord:"Jesus", exhaleWord:"Give Me Peace", journeySteps:[{name:"Praise",prompt:"What attribute of God fills your heart with gratitude today?",verse:"",ref:""},{name:"Surrender",prompt:"What are you ready to place in God's hands?",verse:"",ref:""},{name:"Ask",prompt:"What would you like to bring before God?",verse:"",ref:""},{name:"Listen",prompt:"Be still and listen.",verse:"",ref:""}], ambience:{rain:0,stream:0,fire:0,breeze:15}, icon:"sparkles", verses:[] } },
  ...
  function fieldMarkup(field, data) {
    ...
    if (type==="journey") { const steps=Array.isArray(value)?value:[]; return `<div class="cs-field cs-builder-field wide" data-key="${key}" data-state="complete"><label>${esc(label)}</label><div class="cs-builder-grid">${["Praise","Surrender","Ask","Listen"].map((stage,index)=>{const step=steps[index]||{};return `<fieldset><legend>${index+1}. ${stage}</legend><input name="journeyName${index}" value="${esc(step.name||stage)}" placeholder="Stage name" /><input name="journeyPrompt${index}" value="${esc(step.prompt||"")}" placeholder="What should people reflect on?" /><input name="journeyVerse${index}" value="${esc(step.verse||"")}" placeholder="Scripture text (optional)" /><input name="journeyRef${index}" value="${esc(step.ref||"")}" placeholder="Reference (optional)" /></fieldset>`;}).join("")}</div><small>${esc(hint||"")}</small></div>`; }
    if (type==="ambience") { const mix=value&&typeof value==="object"?value:{}; return `<div class="cs-field cs-builder-field wide" data-key="${key}" data-state="complete"><label>${esc(label)}</label><div class="cs-slider-grid">${["rain","stream","fire","breeze"].map(sound=>`<label>${sound[0].toUpperCase()+sound.slice(1)}<input name="ambience${sound[0].toUpperCase()+sound.slice(1)}" type="range" min="0" max="100" value="${Number(mix[sound]||0)}" /><output>${Number(mix[sound]||0)}%</output></label>`).join("")}</div><small>${esc(hint||"")}</small></div>`; }
    if (type==="verses") { const verses=Array.isArray(value)?value:[]; return `<div class="cs-field cs-builder-field wide" data-key="${key}" data-state="complete"><label>${esc(label)}</label><div class="cs-builder-grid">${[0,1,2].map(index=>{const verse=verses[index]||{};return `<fieldset><legend>Scripture ${index+1}</legend><input name="verseTopic${index}" value="${esc(verse.topic||"")}" placeholder="Topic (optional)" /><textarea name="verseText${index}" placeholder="Scripture text">${esc(verse.text||"")}</textarea><input name="verseRef${index}" value="${esc(verse.ref||"")}" placeholder="Reference (optional)" /></fieldset>`;}).join("")}</div><small>${esc(hint||"")}</small></div>`; }
    if (type==="color") { const colors=["#4a5d3f","#2a8f89","#b86b5c","#5b7052","#d97706","#3157c9"]; return `<div class="cs-field" data-key="${key}" data-state="${value?"complete":"missing"}"><label>${esc(label)}${required?" <em>*</em>":""}</label><div class="cs-color-options">${colors.map(color=>`<button type="button" class="cs-color-swatch${String(value).toLowerCase()===color?" selected":""}" data-color-value="${color}" style="--swatch:${color}" aria-label="${color}"></button>`).join("")}<label class="cs-custom-color"><input type="color" value="${esc(value||colors[0])}" data-custom-color aria-label="Custom accent color" /></label></div><input type="hidden" name="${key}" value="${esc(value)}" data-color-value-input />${hint?`<small>${esc(hint)}</small>`:""}</div>`; }
    ...
    if (type==="audio") return `<div class="cs-field cs-audio-field ${wide?"wide":""}" data-key="${key}" data-state="${value?"complete":"complete"}"><label for="cs-${key}">${esc(label)}</label><input id="cs-${key}" class="cs-audio-picker" type="file" accept="audio/mpeg,.mp3" data-audio-picker /><input type="hidden" name="${key}" value="${esc(value)}" data-audio-value />${value?`<audio controls preload="none" src="${esc(value)}"></audio>`:""}<small>${esc(hint||"MP3 only, up to 10 MB.")}</small><p data-upload-status></p></div>`;
  ...
  function formData(form) { const value=Object.fromEntries(new FormData(form)); form.querySelectorAll('input[type="checkbox"]').forEach(input=>value[input.name]=input.checked); for (const [key,fieldValue] of Object.entries(value)) if (["price","compareAt","inventory","totalTickets","ticketPriceCents","durationMinutes","toneFreq","autoPlayInterval","themeHue"].includes(key)) value[key]=Number(fieldValue||0); if(activeKind==="meditation") { value.journeySteps=[0,1,2,3].map(index=>({name:value[`journeyName${index}`]||["Praise","Surrender","Ask","Listen"][index],prompt:value[`journeyPrompt${index}`]||"",verse:value[`journeyVerse${index}`]||"",ref:value[`journeyRef${index}`]||""})); value.ambience={rain:Number(value.ambienceRain||0),stream:Number(value.ambienceStream||0),fire:Number(value.ambienceFire||0),breeze:Number(value.ambienceBreeze||0)}; value.verses=[0,1,2].map(index=>({topic:value[`verseTopic${index}`]||"",text:value[`verseText${index}`]||"",ref:value[`verseRef${index}`]||""})).filter(item=>item.text||item.topic||item.ref); Object.keys(value).filter(key=>/^(journey|ambience|verse)/.test(key)&&key!=="verses").forEach(key=>delete value[key]); if (value.timeMode === "teaching") value.purpose = value.purpose || "teaching"; } if(activeKind==="resources" && value.access==="Free") value.price=0; return value; }
  ...
  async function uploadAudio(input) {
    const audio=input.files?.[0]; if(!audio) return;
    const field=input.closest(".cs-audio-field"), value=field.querySelector("[data-audio-value]"), status=field.querySelector("[data-upload-status]");
    if(audio.type !== "audio/mpeg" && !audio.name.toLowerCase().endsWith(".mp3")) { status.textContent="Choose an MP3 audio file."; status.className="error"; input.value=""; return; }
    if(audio.size > 10*1024*1024) { status.textContent="Choose an MP3 smaller than 10 MB."; status.className="error"; input.value=""; return; }
    status.textContent="Uploading your audio..."; status.className="uploading"; pendingUploads.add(input); updateReadiness();
    try { const body=new FormData(); body.append("audio",audio); const response=await fetch("/api/media/audio-upload",{method:"POST",body,credentials:"same-origin"}); const result=await response.json().catch(()=>null); if(!response.ok||!result?.ok||!result.url) throw new Error(result?.error||"We could not upload that audio."); value.value=result.url; field.querySelector("audio")?.remove(); field.insertAdjacentHTML("beforeend",`<audio controls preload="none" src="${esc(result.url)}"></audio>`); status.textContent="Audio ready."; status.className="success"; } catch(error) { status.textContent=error.message||"We could not upload that audio."; status.className="error"; } finally { pendingUploads.delete(input); updateReadiness(); }
  }
  ...
  function renderEditor() {
    ...
    const form=root.querySelector("#cs-editor-form"); form.addEventListener("input",updateReadiness); form.addEventListener("change",updateReadiness); form.querySelectorAll("[data-image-picker]").forEach(input=>{ input.addEventListener("change",()=>uploadImage(input)); input.closest(".cs-image-field").querySelector(".cs-image-drop").addEventListener("click",()=>input.click()); }); form.querySelectorAll("[data-audio-picker]").forEach(input=>input.addEventListener("change",()=>uploadAudio(input))); form.querySelectorAll("[data-color-value]").forEach(button=>button.addEventListener("click",()=>{ const field=button.closest(".cs-field"); field.querySelector("[data-color-value-input]").value=button.dataset.colorValue; field.querySelector("[data-custom-color]").value=button.dataset.colorValue; field.querySelectorAll("[data-color-value]").forEach(item=>item.classList.toggle("selected",item===button)); updateReadiness(); })); form.querySelectorAll("[data-custom-color]").forEach(input=>input.addEventListener("input",()=>{ const field=input.closest(".cs-field"); field.querySelector("[data-color-value-input]").value=input.value; field.querySelectorAll("[data-color-value]").forEach(item=>item.classList.toggle("selected",item.dataset.colorValue===input.value)); updateReadiness(); }));
