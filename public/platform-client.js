...
(function (root) {
 ...
 function install(){
  ...
  for(const [kind,mod]of Object.entries(root.MWEAdmin?.modules||{})){
   ...
   mod.save=async(record,values)=>{const existing=workspace.find(row=>row.kind===kind&&row.id===record.id);if(existing&&!existing.canManage)throw new Error('You have read-only access to this record.');platform.staging=true;let transformed;try{transformed=original(record,values);}finally{platform.staging=false;}const saved=await platform.save(kind,{...transformed,publicationState:values.publicationState});if(kind==='products'&&platform.role==='owner'&&saved.storeProductId){const status=values.publicationState==='published'?'published':values.publicationState==='archived'?'archived':values.publicationState==='pending'?'pending':'draft';await api('store/admin/products/'+encodeURIComponent(saved.storeProductId),{status},'PATCH');}return saved;};
  }
  document.addEventListener('click',event=>{const button=event.target.closest('#create-channel-button,#create-resource-button,#create-room-button,#add-product-button,[data-edit-product],[data-delete-product]');if(button){event.preventDefault();event.stopImmediatePropagation();const kind=button.matches('#create-channel-button')?'channels':button.matches('#create-resource-button')?'resources':button.matches('#create-room-button')?'meditation':'products';location.href='app.html?view=create&kind='+encodeURIComponent(kind);}},true);
