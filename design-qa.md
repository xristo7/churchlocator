...
## Active livestream card restoration
...
- Prototype: the active livestream directory rendered in the member shell at 1280 × 800 and 390 × 844.
- Frame and silhouette: passed with a 16:9 landscape card, 26px radius, and 3px light border.
- Image treatment: passed with the actual video thumbnail where available, a creator image fallback, and a dark readability layer.
- Hierarchy: passed with title and description at the upper left.
- Action: passed with a white Play pill at the lower left and no price.
- Responsiveness: passed with three desktop cards, one-column mobile cards, and no horizontal overflow.
- Interaction: passed; each card opens its dedicated broadcast page and supports keyboard focus.
