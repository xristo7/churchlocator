...
test("browser logout never announces local sign-out while server revocation has failed", async () => {
  const source = await readFile(new URL("../public/auth-client.js", import.meta.url), "utf8");
  const values = new Map([["mwe.userLoggedIn", "true"], ["mwe.creator.account.v1", "cached"]]);
  let accepted = false;
  const window = {};
  const context = vm.createContext({ window, document: { addEventListener() {} }, localStorage: { removeItem: key => values.delete(key), getItem: key => values.get(key), setItem: (key, value) => values.set(key, value) }, fetch: async () => ({ ok: accepted, json: async () => ({ ok: accepted }) }) });
  vm.runInContext(source, context);
  assert.equal((await window.MWEAuth.logout()).ok, false);
  assert.equal(values.get("mwe.userLoggedIn"), "true");
  accepted = true;
  assert.equal((await window.MWEAuth.logout()).ok, true);
  assert.equal(values.has("mwe.userLoggedIn"), false);
  assert.equal(values.has("mwe.creator.account.v1"), false);
});
