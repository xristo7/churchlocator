...
async function registerUser(request, env, { forceCreator = false } = {}) {
  ...
  if (password.length < 12 || password.length > 128) return json({ ok: false, error: "Password must be between 12 and 128 characters." }, 400);
...
async function handleEventRegister(request, env) {
  ...
  if (result.meta?.changes !== 1) return json({ ok: false, error: "Registration unavailable or event full." }, 409);
