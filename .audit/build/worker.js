...
function validateValue(value, key = "", depth = 0) {
  ...
  if (typeof value === "string") {
    const maximum = /^avatarData$/i.test(key) ? 7e5 : /password/i.test(key) ? 128 : /email/i.test(key) ? 254 : /^(id|.*Id)$/i.test(key) ? 128 : /name|title|phone/i.test(key) ? 200 : 1e4;
...
var taxonomyKeys = ["denominations", "languages", "worship_styles", "store_categories", "product_categories", "channel_topics", "resource_topics"];
var privateKinds = ["prayer", "reflection", "ride", "visit", "salvation", "foundation", "message", "settings"];
var livestreamKinds = { church: "churches", channel: "channels", store: "store" };
...
__name(whole, "whole");
var pastoralRoles = /* @__PURE__ */ new Set(["owner", "editor", "pastor"]);
function requiredText(value, label, maximum = 5e3) {
...
function privateSubmission(kind, input, entity) {
  ...
  const data = Object.fromEntries(allowed.filter((key) => input[key] !== void 0).map((key) => [key, sanitizeValue(input[key], key)]));
  if (entity) {
  ...
  if (kind === "ride") {
    data.fullName = requiredText(data.fullName, "Full name", 200);
    data.phone = requiredText(data.phone, "Phone", 200);
    data.email = String(data.email || "").trim().toLowerCase();
    if (!validEmail(data.email)) throw new ApiError(400, "Enter a valid email address.");
    data.pickupAddress = requiredText(data.pickupAddress, "Pickup address", 1e3);
    data.passengers = whole(data.passengers || 1, "Passengers", 10);
    if (data.passengers < 1) throw new ApiError(400, "At least one passenger is required.");
    if (data.consent !== true && data.consent !== "true") throw new ApiError(400, "Contact consent is required for ride coordination.");
    data.consent = true;
    data.stage = 1;
    data.stage1Confirmed = false;
    data.stage2Confirmed = false;
    data.driver = "Unassigned";
    data.pickupWindow = "Pending availability verification";
  }
  if (kind === "visit") {
    data.firstName = requiredText(data.firstName, "First name", 200);
    data.lastName = requiredText(data.lastName, "Last name", 200);
    data.email = String(data.email || "").trim().toLowerCase();
    if (!validEmail(data.email)) throw new ApiError(400, "Enter a valid email address.");
    data.phone = requiredText(data.phone, "Phone", 200);
    data.gathering = requiredText(data.gathering, "Gathering", 300);
  ...
  if (kind === "salvation") {
    ...
    data.email = String(data.email || "").trim().toLowerCase();
    if (!validEmail(data.email)) throw new ApiError(400, "Enter a valid email address.");
    data.needBible = boolean(data.needBible);
    data.needPrayer = boolean(data.needPrayer);
  }
  return data;
}
__name(privateSubmission, "privateSubmission");
...
__name(checkTaxonomyValues, "checkTaxonomyValues");
...
async function handlePlatformApi(request, env, ctx) {
  ...
  if (!["catalog", "taxonomies", "workspace", "private", "admin-audit", "tenant-members", "resource-material", "livestream-chat"].includes(scope)) return null;
  if (!env.DB) throw new ApiError(503, "Storage unavailable.");
  if (scope === "livestream-chat") {
    ...
    if (request.method === "GET") {
      const { results } = await env.DB.prepare(`select m.id,m.user_id,m.body,m.created_at,u.name,u.avatar_url
    ...
    order by m.created_at desc,m.id desc limit 100`).bind(entityId, streamType).all();
      const messages = (results || []).reverse().map((row) => ({ id: row.id, userId: row.user_id, name: row.name, avatarUrl: row.avatar_url || "", body: row.body, createdAt: row.created_at }));
      return ctx.json({ ok: true, messages });
    }
    if (request.method === "POST") {
      const user2 = await requireUser(request, env, ctx), input = await readJson(request), body = String(input.body || "").trim();
      if (!body || body.length > 300) throw new ApiError(400, "Chat messages must be between 1 and 300 characters.");
      const id = crypto.randomUUID(), createdAt = (/* @__PURE__ */ new Date()).toISOString();
      await env.DB.prepare("insert into livestream_chat_messages (id,entity_id,stream_type,user_id,body,created_at) values (?,?,?,?,?,?)").bind(id, entityId, streamType, user2.id, body, createdAt).run();
      return ctx.json({ ok: true, message: { id, userId: user2.id, name: user2.name, avatarUrl: user2.avatar_url || "", body, createdAt } }, 201);
    }
    throw new ApiError(405, "Method not allowed.");
  }
...
var GOOGLE_OAUTH_STATE_COOKIE = "mwe_google_oauth_state";
var SESSION_TTL_SECONDS = 60 * 60 * 24;
var GOOGLE_OAUTH_STATE_TTL_SECONDS = 10 * 60;
...
__name(oauthStateCookieHeader, "oauthStateCookieHeader");
...
__name(redirectWithCookies, "redirectWithCookies");
...
function validAvatarDataUrl(value) {
  ...
  if (value.length > 7e5) return false;
  return /^data:image\/(?:png|jpeg|webp);base64,[A-Za-z0-9+/]+={0,2}$/.test(value);
}
__name(validAvatarDataUrl, "validAvatarDataUrl");
async function verifyUserPassword(user, password, env) {
  ...
  const computed = modern ? await modernPassword(password, user.password_salt) : (versioned ? "pbkdf2-sha256$100000$" : "") + await hashPassword(password, user.password_salt);
  return constantTimeEqual(computed, user.password_hash);
}
__name(verifyUserPassword, "verifyUserPassword");
async function handleAuthProfileUpdate(request, env) {
  ...
  await env.DB.prepare("update users set name = ?, email = ?, avatar_url = ?, email_verified_at = case when email <> ? then null else email_verified_at end, updated_at = ? where id = ?").bind(name, email, avatarUrl || null, email, (/* @__PURE__ */ new Date()).toISOString(), user.id).run();
...
__name(handleAuthProfileUpdate, "handleAuthProfileUpdate");
async function handleAuthPasswordUpdate(request, env) {
  ...
  await env.DB.prepare("update users set password_hash = ?, password_salt = ?, updated_at = ? where id = ?").bind(next.hash, next.salt, (/* @__PURE__ */ new Date()).toISOString(), user.id).run();
  return json({ ok: true, message: "Password updated." });
}
__name(handleAuthPasswordUpdate, "handleAuthPasswordUpdate");
...
__name(handleGoogleAuthStart, "handleGoogleAuthStart");
...
__name(parseGoogleOauthState, "parseGoogleOauthState");
async function handleGoogleAuthCallback(request, env) {
  ...
  if (!user) {
    ...
    await env.DB.prepare("update users set last_login_at = ?, email_verified_at = coalesce(email_verified_at, ?) where id = ?").bind(now, now, user.id).run();
...
__name(handleGoogleAuthCallback, "handleGoogleAuthCallback");
...
async function handlePrayer(request, env) {
  ...
  if (requestText.length < 3 || requestText.length > 5e3) return json({ ok: false, error: "request must be between 3 and 5000 characters" }, 400);
  if (payload.churchId) {
    const church = await env.DB.prepare("select id from churches where id = ? and is_verified = 1").bind(payload.churchId).first();
    if (!church) return json({ ok: false, error: "published church not found" }, 404);
  }
